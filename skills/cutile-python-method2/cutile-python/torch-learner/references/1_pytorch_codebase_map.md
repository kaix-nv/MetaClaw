...
**IMPORTANT:** All paths in this document are relative to the cloned PyTorch source at `.cache/pytorch-source/`. For example, `torch/nn/modules/` means `.cache/pytorch-source/torch/nn/modules/`. ALL searches must be scoped to `.cache/pytorch-source/` — never search outside this directory.
...
```bash
PYTORCH_SRC=.cache/pytorch-source
OP_NAME=lstm
