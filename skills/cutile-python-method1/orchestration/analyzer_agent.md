...
- Existing code (PyTorch/Triton) to port to cuTile
...
Before producing your decomposition, read these files for context on cuTile's capabilities and constraints. All files are under `<skill_dir>/references/` (use the `Skill directory` path from your prompt).

- `<skill_dir>/references/01_overview_concepts.md` - Understand what cuTile can and cannot do
- `<skill_dir>/references/15_matmul_ops.md` - If matmul operations are involved
- `<skill_dir>/references/13_reduction_ops.md` - If reductions are involved
- `<skill_dir>/guidelines/04_tile_programming.md` - Tile-based programming philosophy (affects fusion decisions)
