"""Case 1: Flash Attention without Causal Mask with the input shape (B, S, H, D)"""
import torch
import numpy as np
...
def fmha_no_causal_mask_kernel(
    Q, K, V, O,
    sm_scale,
    batch_size: ct.Constant[int], seq_len: ct.Constant[int], num_heads: ct.Constant[int], head_dim: ct.Constant[int],
    ...
    # Load Q block
    q = ct.load(Q, index=(bid_batch, bid_seq, bid_head, 0), shape=(1, BLOCK_M, 1, BLOCK_D))
    ...
    for start_n in range((seq_len + BLOCK_N - 1) // BLOCK_N):
        # Load K block, shape: (1, BLOCK_N, 1, BLOCK_D)
        k = ct.load(K, index=(bid_batch, start_n, bid_head, 0), shape=(1, BLOCK_N, 1, BLOCK_D))
        ...
        # Load V block, shape: (1, BLOCK_N, 1, BLOCK_D)
        v = ct.load(V, index=(bid_batch, start_n, bid_head, 0), shape=(1, BLOCK_N, 1, BLOCK_D))
    ...
    # Normalize output
    o = o / l
    o = ct.reshape(o, (1, BLOCK_M, 1, BLOCK_D))
    o = ct.astype(o, O.dtype)

    # Write output
    ct.store(O, index=(bid_batch, bid_seq, bid_head, 0), tile=o)

def fmha(q, k, v, sm_scale):
    batch_size, seq_len, num_heads, head_dimension = q.shape
    assert k.shape == v.shape == (batch_size, seq_len, num_heads, head_dimension)
    ...
    ct.launch(torch.cuda.current_stream(), grid, fmha_no_causal_mask_kernel, (
        q, k, v, o,
        sm_scale,
        batch_size, seq_len, num_heads, head_dimension,
...
q = torch.rand(batch_size, seq_len, num_heads, head_dimension, device='cuda', dtype=torch.float16)
k = torch.rand(batch_size, seq_len, num_heads, head_dimension, device='cuda', dtype=torch.float16)
v = torch.rand(batch_size, seq_len, num_heads, head_dimension, device='cuda', dtype=torch.float16)
